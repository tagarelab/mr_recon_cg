...
sim_noisy_corrected = phase_shift_by_segments(sim_noisy_sig[samp_mask], 70)

img_fft = np.reshape(sim_noisy_corrected, (N_echoes, -1))
plt.figure()
plt.imshow(np.abs(cs.freq2im(img_fft, theta=theta)), vmin=0, vmax=1)
plt.title("Corrected")
plt.show()
...
sim_noisy_corrected2 = cs.phase_shift_by_segments(sim_noisy_sig[samp_mask], echo_len, temp[::echo_len])

img_fft = np.reshape(sim_noisy_corrected2, (N_echoes, -1))
plt.subplot(1, 2, 1)
plt.imshow(np.angle(cs.freq2im(img_fft, theta=theta)), vmin=0, vmax=1)
plt.title('Image Space')
# Display k-space
