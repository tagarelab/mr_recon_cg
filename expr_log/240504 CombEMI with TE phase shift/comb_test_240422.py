...
def phase_shift_back_by_segments(x, t, shift_by):
    """
    Subtract the first t points by x[0], then the next t points by x[t], and so on.
    """
    # Ensure x is a numpy array
...
def plot_img_freq_spaces(signal, N_echoes, theta, name='Image Space'):
    img_fft = np.reshape(signal, (N_echoes, -1))
    plt.subplot(2, 2, 1)
    ...
    plt.title('K-space')
    plt.subplot(2, 2, 3)
    plt.imshow(np.angle(freq2im(img_fft, theta=theta)), vmin=-np.pi, vmax=np.pi)
    plt.title('Angle')
    # Display k-space
    plt.subplot(2, 2, 4)
