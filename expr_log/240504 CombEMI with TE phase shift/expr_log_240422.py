...
for k in range(N_rep):
    ...
    signal_shifted = cs.phase_shift_back_by_segments(signal, echo_len, signal[0::echo_len])

    cancelled_comb_raw = cs.comb_optimized(signal=signal_shifted, N_echoes=N_echoes, TE=TE, dt=dt,
    ...
    cs.plot_img_freq_spaces(cancelled_comb, theta=theta, N_echoes=N_echoes, name='Comb Raw Output')

    # temp = str_noi[samp_mask_w_pol]
    # cancelled_comb = cs.phase_shift_by_segments(cancelled_comb, echo_len, signal[0::echo_len])

    # cs.plot_img_freq_spaces(signal, theta=theta, N_echoes=N_echoes, name='Input Signal')
    # cs.plot_img_freq_spaces(temp, theta=theta, N_echoes=N_echoes, name='EMI')
    # cs.plot_img_freq_spaces(cancelled_comb, theta=theta, N_echoes=N_echoes, name='Comb Shifted Output')
    ...
    temp = signal_shifted - cancelled_comb
    temp = cs.phase_shift_by_segments(temp, echo_len, signal[0::echo_len])
    sig_comb = np.reshape(temp[cs.calculate_polar_period(polar_time=polar_time, dt=dt):], (N_echoes, -1))
