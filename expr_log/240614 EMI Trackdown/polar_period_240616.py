...
sig_all_steps = np.zeros((len(file_name_steps), int(np.ceil(N_echoes_real * TE_real / dt))), dtype='complex')
pol_all_steps = np.zeros((len(file_name_steps), int(np.ceil(N_rep_pol * rep_time_pol / dt))), dtype='complex')
# sig_all_steps = []
# pol_all_steps = []
...
for stem_id in range(len(file_name_stem)):
    for step_id in range(len(file_name_steps)):
        file_name = file_name_stem[stem_id] + file_name_steps[step_id]
        print("Processing File ", file_name)
        mat_file = sp.io.loadmat('sim_input/' + file_name + '.mat')
        if file_name_stem[stem_id] == 'Sig_3AxisEMI_':
            channel_info = channel_info_1
        elif file_name_stem[stem_id] == 'SigOnly_':
            channel_info = [""]
        for channel_id in range(len(channel_info)):
            ...
            for i in [0]:
                ...
                for seg_id in range(len(segment_names)):
                    ...
                    if seg_id == 0:
                        pol_all_steps[step_id, :] = np.where(samp_all, zfilled_data, 0)
                        # pol_all_steps.append(zfilled_data.copy())
                    elif seg_id == 1:
                        sig_all_steps[step_id, :] = np.where(samp_all, zfilled_data, 0)
...
# dict = {"sig_all_steps": sig_all_steps, "pol_all_steps": pol_all_steps}
