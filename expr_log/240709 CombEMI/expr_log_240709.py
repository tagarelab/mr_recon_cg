...
# %% Create comb shaped masks
noi_mask_TE, sig_mask_TE, samp_mask_TE = cs.gen_masks(acq_len=echo_len_real, N_echoes=N_echoes_real, TE=TE_real,
                                                      pk_id=65, polar_time=0,
                                                      post_polar_gap_time=0, dt=dt, pk_win=0.33)
noi_mask_pol, sig_mask_pol, samp_mask_pol = cs.gen_masks(acq_len=rep_len_pol, N_echoes=N_rep_pol, TE=rep_time_pol,
                                                         pk_id=65,
                                                         polar_time=0,
                                                         post_polar_gap_time=0, dt=dt, pk_win=0.33)
...
stem_id = 0
channel_id = 1  # signal channel only
#
for stem_id in [0, 1]:
    vis.compare_unnormalized_freq(signal_list=sig_all_steps[channel_id, stem_id, step_ids, :],
                                  legends=file_name_steps[step_ids],
                                  dt=dt,
                                  name=channel_info[channel_id] + '_TE Acquisition_' + file_name_stem[stem_id],
                                  subplot=True, ylim=ylim_sig)
    vis.compare_unnormalized_freq(signal_list=pol_all_steps[channel_id, stem_id, step_ids, :],
                                  legends=file_name_steps[step_ids], dt=dt,
                                  name=channel_info[channel_id] + '_Pre Polarization' + file_name_stem[
                                      stem_id], subplot=True, ylim=ylim_prepol)

# %% Masked Signal
for stem_id in [0, 1]:
    vis.compare_unnormalized_freq(signal_list=sig_all_steps[channel_id, stem_id, step_ids, :] * noi_mask_TE,
                                  legends=file_name_steps[step_ids],
                                  dt=dt,
                                  name=channel_info[channel_id] + '_TE Acquisition_Noise_' + file_name_stem[stem_id],
                                  subplot=True, ylim=ylim_sig)

    vis.compare_unnormalized_freq(signal_list=sig_all_steps[channel_id, stem_id, step_ids, :] * sig_mask_TE,
                                  legends=file_name_steps[step_ids],
                                  dt=dt,
                                  name=channel_info[channel_id] + '_TE Acquisition_Signal_' + file_name_stem[stem_id],
                                  subplot=True, ylim=ylim_sig)

    vis.compare_unnormalized_freq(signal_list=sig_all_steps[channel_id, stem_id, step_ids, :] * samp_mask_TE,
                                  legends=file_name_steps[step_ids],
                                  dt=dt,
                                  name=channel_info[channel_id] + '_TE Acquisition_Samp_' + file_name_stem[stem_id],
                                  subplot=True, ylim=ylim_sig)
