...
def comb_optimized(signal, N_echoes, TE, dt, lambda_val, tol, max_iter, pre_drop, post_drop, pk_win,
    ...
    if pk_id is None:  # use input ID if otherwise
        if pk_id == "mid":
            ...
            # pks[:5, :] = 0  # sometimes there's a leak of signal at the beginning
    ...
    # plt.figure()
    # plt.plot(np.abs(zfilled_data[0:400]), label="abs")
    # plt.plot(np.abs(noi_all[0:400] * 100000), label="abs")
    # # plt.legend()
    # plt.show()
    ...
    while lambda_val > 0:
        ...
        # lambda_val = -1     # TODO: remove this for multi-loop auto lambda
